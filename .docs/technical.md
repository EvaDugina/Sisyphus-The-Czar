...
6. Клиентский `createWindowObstacleController` считает высоту центра камня, управляет popup lifecycle и блокирует только input при наличии активных obstacle-окон; preclick-окна картин живут независимо, используют общий размер последовательности, а по первому настоящему клику получают повторную геометрию и `focus()`. Отдельный одноразовый таймер затем вызывает `blur()` у всех живых preclick-popup и `focus()` главного окна, не закрывая popup; отложенное окно, появившееся после таймера, сразу получает то же состояние. `createRockEchoTrailController` сцены 1 событийно семплирует движение DOM-камня и управляет ограниченным набором затухающих копий без собственного rAF. Серверный fixed-step цикл продолжает работать.
...
8. `sceneFlow` фиксирует `completed`, `completionReason` и `finalFallStarted`. Сцена 1 в режиме `preclickFirstHopOnClick=true` сначала перехватывает отдельный pointerdown для начального hop, затем после N фейковых кликов вызывает `completeScene("first-real-rock-press")` на следующем pointerdown; сцена 2 стартует suspended в центре viewport и вызывает `completeScene("rock-touched-imprint")` при первом пересечении отпечатка; сцена 3 на первом пересечении синхронно скрывает локальную и shared-pointer руку, вызывает `sceneThree.lock`, центрирует камень по imprint и блокирует повторный захват. Только trusted wheel/touch/key scroll вниз отправляет `sceneThree.release` и запускает final fall; viewport остаётся неподвижным, а завершение наступает при приземлении камня после затухания дождя и его громкости.
...
- `updatePreclickRockHop()` всегда обновляет координаты и скорость указателя, но при `preclickFirstHopOnClick=true && hopCount===0` не вызывает radius-hop. Первый валидный pointerdown проходит через `activatePreclickFirstHopOnClick()` раньше `consumePreclickGuardClick()`: выполняет hop, увеличивает `radiusHopCount`, помечает решение `click-trigger`, но оставляет `guardClicksUsed=0` и не открывает popup. После активации каждый новый вход мыши в радиус `preclickHopActivationRadiusPercent / 100 × getBoundingClientRect().width` проходит через `preclickRadiusHopDecision()`: первые два успешных hop с учётом начального обязательны, следующая попытка получает единственный `forced-miss`, затем решение использует `preclickHopMissProbabilityPercent / 100`. Выход из радиуса взводит следующую проверку; restart сбрасывает счётчики. `preclickDirectionalViewportSpan()` выбирает единицу пути по вектору и пропорциям viewport, а скорость указателя `0–2000 px/s` линейно отображается в `0.28–1.0` от UI-максимума. Перед анимацией `preclickHopPathIsSafe()` непрерывно проверяет отрезок против повторяющихся на торе копий зоны руки, а `preclickToroidalDistance()` исключает визуальный возврат в старт. Кандидаты перебираются по ближайшим углам и уменьшениям дистанции, не превышая UI cap. Длительность вычисляет `preclickHopDurationMs(pathLength, preclickHopSpeedPxPerSecond)`, прогресс использует `preclickHopSpeedEasing`; покадровый wrap, reduced motion и resize сохраняются, а каждый реально начатый hop запускает выбранный preclick-звук.
- `goghArtworks.mjs` через Vite `import.meta.glob` строит каталог всех поддерживаемых raster-файлов `assets/gogh`, а `goghArtworkSelection.mjs` стабильно сортирует basename-ID, удаляет дубли и реализует `random`, shuffle-bag без повторов внутри цикла и `single` с fallback на первый элемент каталога. Пока `preclickRockGuidance.completed=false`, физика остаётся suspended, hop активен, `html/body.is-manual-scroll-disabled` блокируют ручной vertical scroll. `consumePreclickGuardClick()` перехватывает первые два валидных pointerdown после проверки фазы и obstacle-window, увеличивает только `guardClicksUsed`, выбирает картину через selector, ставит `openPreclickWindow()` в timeout `preclickPopupDelayMs` и выполняет hop без `playGachiClickSound()`. Каждый выполненный hop независимо запускает `Смех.mp3` и передаёт изменившуюся DOM-геометрию в эхо-контроллер. Первый успешно открытый popup вычисляет запрошенную ширину как `innerWidth × preclickPopupWidthViewportFraction`, высоту — по aspect ratio первой картины и фиксирует `sharedPreclickPopupSize`; все следующие окна игнорируют собственные width/aspect-ratio при выборе габарита и растягивают `<img>` на общий клиентский прямоугольник. `preclickPopupEdgeSize()` пропорционально уменьшает пару, если она не помещается ни в одну из четырёх периферийных полос вокруг центральных 50% видимой области приложения. После `window.open()` контроллер измеряет browser chrome, повторно вычисляет outer-safe геометрию и вызывает `resizeTo()`/`moveTo()`. Для второго финального окна `preclickPopupEdgeGeometry()` детерминированно применяет один набор random seeds к допустимой левой, верхней, правой или нижней зоне, поэтому весь внешний прямоугольник не пересекает центр и не выходит за границы приложения. Preclick-окно не получает auto-close/click-close, не входит в `activeObstacleCount()` и после открытия не закрывается dispose; pending timeout при dispose отменяется. `revealPreclickWindows()` помечает reveal-состояние, повторно подгоняет и фокусирует все живые окна; окно из ранее поставленного timeout тоже фокусируется сразу после появления. Третий клик основной последовательности без смеха/gachi материализует wrapped-центр, сначала вызывает reveal, затем открывает два новых окна общего размера — у pointer и в edge-зоне — вызывает `schedulePreclickWindowsBackground(preclickPopupBackgroundDelaySeconds)` и выполняет `completeScene("first-real-rock-press")`. Контроллер допускает только один активный background timeout. По его истечении `backgroundPreclickWindows()` оставляет окна открытыми, вызывает у них `blur()` и возвращает фокус главному окну; если отложенный popup открылся позднее, `trackWindow()` немедленно применяет к нему background-состояние. Обычная траектория сцены 1 не включается, а restart/dispose очищает таймер и сбрасывает shuffle-bag.
- Scene-2 scale lifecycle хранит `sceneTwoSizeState` и `sceneTwoSizeCycleArmed`. Успешный захват взводит новый цикл и вызывает `beginSceneTwoGrabScale()`, останавливая пульс. Обычные `pointerup`, `pointercancel`, `lostpointercapture` и уход окна в blur игнорируются, пока камень удерживается в сцене 2; фактическое освобождение выполняют только серверные `randomDropEnabled` и `rockJumpEnabled`, после чего состояние переходит в `airborne` и пульс возобновляется. `settleSceneTwoRockScaleOnGround()` на первом ground-touch ставит `ground`, снимает флаг и сохраняет пульс активным. Повторные отскоки при снятом флаге не взводят size-cycle; следующий цикл начинается только с нового успешного захвата. `SceneTwoPage` наблюдает снятие runtime-класса `is-dragging` и сбрасывает локальную sticky-защёлку после фактического release. `gachiClickSoundFilename="none"` и `wallImpactSoundFilename="none"` проходят shared sanitization и завершают соответствующее воспроизведение до создания `Audio`; wall-impact выбирает собственный loader и срабатывает один раз на новый контакт. Remote snapshot использует ту же state-machine; camera-follow независимо применяет пары up/down enabled+lerp.
...
- Scene-pages smoke проверяет точные `39 / 104 / 106` контролов и `20 / 84 / 84` общих маркера, каскадную миграцию, двустороннее восстановление общих значений после перехода между URL, сохранение уникального поля, режим и условный select Gogh, отсутствие скрытых scene-1 полей, desktop resize, отдельный wheel-scroll без изменения `window.scrollY`, отсутствие обрезания значений, narrow viewport и завершение сцен 2/3.
...
- Постоянный preclick-hop smoke проверяет guidance, scroll lock, reload/restart, радиус от текущей ширины камня, два обязательных radius-hop со смехом, третий forced miss, поведение `0%`, скорость/кривую, дискретный эхо-след, отложенный popup с картинами `01–03`, отсутствие auto-close, отсутствие обычной траектории в сцене 1, направленный максимум, безопасный X/Y/corner wrap, resize, reduced motion и кликабельность. Отдельный Chromium-сценарий настоящей UI-кнопкой включает режим `клик`, подтверждает отсутствие hop по hover и проходит цепочку «активационный клик → два фейковых → настоящий» с неизменным `guardClicksUsed` на активации. Основной popup-сценарий подтверждает ровно два фейковых клика, отсутствие gachi-звука и `control.acquire`, единый размер старых и двух финальных окон, единственный background-таймер, неизменную задержку при дополнительных настоящих кликах, `blur()` всех живых popup без закрытия и возврат фокуса главному окну.
