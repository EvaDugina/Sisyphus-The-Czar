...
- Shared room settings schema и settings schema — `44`, localStorage — `sisyphus-czar-settings-v44`; цепочка legacy-ключей начинается с `v43` и сохраняет поддержку более ранних версий. Миграция `v43 → v44` добавляет `sceneTwoOverflowYVisible=false`. Миграция `v42 → v43` по-прежнему добавляет `cameraFollowDownEnabled=true`, `upperZoneAutoScrollEnabled=false` и `gachiClickSoundFilename=Camen.mp3`.
- Категория единственной руки называется «Рука» и содержит `handVisibilityMode` (`always|hover|hidden`, default `always`), `handImageChangeDelayMs` (`0–1000`, integer, default `0`) и `rockGrabRadiusVh`. Категория «Камера» содержит `cameraFollowLerp` (`0.01–1`, default `0.1`), `cameraFollowDownEnabled` (default `true`), `upperZoneAutoScrollEnabled` (default `false`) и `sceneTwoOverflowYVisible` (default `false`). Последний параметр ставит inline `overflow-y: hidden|auto` на `<html>`; программный `window.scrollTo` и `cameraFollowDirectionalScrollY()` продолжают работать при `hidden`. «Препятствия → Окна» содержит девять versioned controls и `WindowObstaclePermissionControl` со статусом и test action.
...
- `trailRenderProfile`, `glowOptimizationMode`, `glowTargetFps`, `glowBufferScalePercent`, `glowUpdateFps`, `glowMaxPoints` и `glowDecimation` имеют `scope: "local"`: сохраняются в `v44`, но фильтруются из version snapshots, server templates и broadcast.
...
- `settings.update` использует schema `44` и optimistic `settingsRevision`.
...
- UI/Fold smoke проверяет legacy-миграции в `v44`, удаление `trailUnlimited`, максимум 10 000, hop-defaults, четыре новых параметра сцены 2, `overflow-y: hidden|auto`, программный scroll при hidden, локальный render-профиль, две сценовые кнопки, scene-specific размеры и общий раздел «Траектория».
