...
- Серверная settings schema — `29`; localStorage key — `sisyphus-czar-settings-v29`, `v28` и более ранние ключи мигрируются как legacy.
- Shared room settings schema — `25`. `preclickParallaxActivationRadiusVw` имеет диапазон `0–200` и default `50`; `preclickParallaxMaxOffsetVw` — диапазон `0–150` и default `0.6`. Старые `preclickParallaxActivationRadiusPx` и `preclickParallaxMaxOffsetPx` мигрируют по фиксированному соотношению `20 px = 1 vw`. `preclickParallaxInverted` по умолчанию `false`, `preclickParallaxReturnDurationMs` (`0–2000`, default `400`) и `preclickParallaxReturnEasing` (default `cubic-bezier(0.22, 1, 0.36, 1)`) сохраняют прежний контракт; старые payload получают безопасные defaults.
- Категория единственной руки называется «Рука» и содержит `handAlwaysVisible` (default `true`). Категория «Камера» содержит `cameraFollowLerp` (`0.01–1`, default `0.1`). «Препятствия → Окна» содержит девять versioned controls и `WindowObstaclePermissionControl` со статусом и test action.
- В группе «Камень» находятся два checkbox, три range-контрола автоматического прыжка и пять parallax-контролов: максимум отклонения, радиус активации, toggle-кнопка инверсии, длительность возврата и cubic-bezier-редактор с графиком.
...
- `glowOptimizationMode`, `glowTargetFps`, `glowBufferScalePercent`, `glowUpdateFps`, `glowMaxPoints` и `glowDecimation` имеют `scope: "local"`: сохраняются в v29, но фильтруются из version snapshots, server templates и broadcast.
- Preclick guidance является штатной функцией dev и production и не зависит от build-time flag. Parallax вычисляет базовый визуальный центр без уже применённого offset. Радиус и максимум в runtime переводятся из `vw` в пиксели как `valueVw / 100 · window.innerWidth`. Внутри круговой зоны направление равно нормализованному вектору `pointer - center`, а длина смещения — `maxOffsetPx · clamp(1 − distance / radius, 0, 1)`; в точном центре сохраняется последнее ненулевое радиальное направление. `preclickParallaxInverted` меняет только знак направления. При выходе указателя за радиус текущий offset через `requestAnimationFrame` интерполируется к нулю по выбранным duration и cubic-bezier. Повторный вход отменяет return-animation и сразу возвращает прямое управление parallax; reduced motion и нулевая duration обнуляют offset без анимации. CSS-композиция камня содержит только `translate3d + scale`, без perspective и rotation.
- До первого допустимого клика `preclickRockGuidance.completed=false`: физика остаётся suspended, parallax активен, `html/body.is-manual-scroll-disabled` блокируют колесо и ручной vertical scroll. Клик по камню помечает guidance завершённым, обнуляет offset, включает физику и camera-follow. `cameraTargetScrollY` ставит документный центр камня на `0.5 · viewportHeight` с clamp по границам документа, а `cameraFollowScrollY` на каждом кадре приближает текущий scroll к цели по `cameraFollowLerp`; старые `positionScroll*` удалены из runtime и schema.
...
- Draft smoke проверяет идентичность основной Fold-сцены и настроек на `/` и `/drafts/`, диапазон максимума parallax в `vw`, режим постоянной/hover-руки, camera lerp, obstacle defaults, popup-blocked UI, сохранение glow-профиля, select-зависимости и безопасную структуру зеркала.
- Dev smoke проверяет миграцию legacy-настроек в v29, включая перевод parallax-радиуса и максимума из px в vw, default-off для инверсии, default-on для постоянной руки, лимит glow-точек, отсутствие проходов при `glow=0` и копирование Fold только при новой canvas revision.
- Отдельный preclick-guidance smoke является постоянным regression-тестом и проверяет статичную камеру до активации, руку сразу после загрузки, рост смещения при приближении к центру, обычное радиальное направление, неизменность размеров камня, отключение parallax первым кликом и движение камеры вслед за drag камня.
