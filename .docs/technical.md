...
8. Runtime сначала передаёт preclick-нажатие в `consumePreclickGuardClick`: если это один из первых N фейковых кликов, выполняется только hop со смехом и click-gachi не создаётся. Каждый кадр preclick-hop передаёт рассчитанную визуальную anchor-точку в общий trail-recorder и ставит session/glow invalidation в единый frame-coordinator; history до checkpoint не перестраивается. Геометрия мира считывается один раз перед анимацией, без принудительного покадрового layout. Для следующего настоящего primary click/tap runtime выбирает gachi только из совпадения shared-манифеста с Vite audio glob, создаёт независимый `Audio` и при переходе в `FALLING` останавливает весь набор активных экземпляров. Успешный новый захват выставляет одноразовый `groundImpactAudio.armed`; первый следующий локальный `touchedGround` или рост серверного `groundTouchSeq` погашает флаг и запускает `СимуляцияОргазма.mov`. Последующие отскоки и sequence jumps без нового захвата не воспроизводят звук; restart и dispose сбрасывают разрешение.
...
- Shared room settings schema и settings schema — `42`, localStorage — `sisyphus-czar-settings-v42`; цепочка legacy-ключей начинается с `v41` и сохраняет поддержку более ранних версий. Миграция `v41 → v42` добавляет `preclickHopMissProbabilityPercent=10`, `preclickHopSpeedPxPerSecond=1200` и `preclickHopSpeedEasing=cubic-bezier(0.22, 1, 0.36, 1)`. Предыдущая миграция `v40 → v41` по-прежнему переводит `trailUnlimited=true` в `trailMaxPoints=10 000` и удаляет legacy-поле.
- Категория единственной руки называется «Рука» и содержит `handVisibilityMode` (`always|hover|hidden`, default `always`), `handImageChangeDelayMs` (`0–1000`, integer, default `0`) и `rockGrabRadiusVh`. Категория «Камера» содержит `cameraFollowLerp` (`0.01–1`, default `0.1`); `cameraFollowScrollUpY()` интерполирует только к меньшему `scrollY`, поэтому камера следует за подъёмом камня, но не прокручивается вниз. «Препятствия → Окна» содержит девять versioned controls и `WindowObstaclePermissionControl` со статусом и test action.
...
- `trailRenderProfile`, `glowOptimizationMode`, `glowTargetFps`, `glowBufferScalePercent`, `glowUpdateFps`, `glowMaxPoints` и `glowDecimation` имеют `scope: "local"`: сохраняются в `v42`, но фильтруются из version snapshots, server templates и broadcast.
...
- `settings.update` использует schema `42` и optimistic `settingsRevision`.
...
- UI/Fold smoke проверяет legacy-миграции в `v42`, удаление `trailUnlimited`, максимум 10 000, новые hop-defaults, локальный render-профиль, две сценовые кнопки, scene-specific размеры и общий раздел «Траектория».
...
- Постоянный preclick-hop smoke проверяет guidance, scroll lock, reload/restart, радиус от текущей ширины камня, два обязательных radius-hop, третий forced miss, поведение `0%`, скорость/кривую, центрированное окно с `ВЗГЛЯД.jpg`, направленный максимум, безопасный X/Y/corner wrap, resize, reduced motion, смех и кликабельность. Отдельные сценарии доказывают создание нескольких canonical/local trail-точек до завершения первой сцены и задают N=3: auto-hop не расходует счётчик и не отправляет `control.acquire`, первые три клика открывают popup и только отталкивают камень без gachi, четвёртый создаёт один gachi, материализует позицию и включает физику, а restart обнуляет счётчик и radius-decision state.
