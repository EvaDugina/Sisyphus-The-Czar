...
- Shared room settings schema и settings schema — `48`, localStorage — `sisyphus-czar-settings-v48`; цепочка legacy-ключей начинается с `v47` и сохраняет поддержку более ранних версий. Миграция `v47 → v48` добавляет `preclickPopupSizeMultiplier=2`, `birchBackgroundEnabled=false` и `birchScalePercent=100`; более ранние миграции по-прежнему добавляют свои отсутствующие defaults.
