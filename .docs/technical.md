...
- Shared room settings schema и settings schema — `45`, localStorage — `sisyphus-czar-settings-v45`; цепочка legacy-ключей начинается с `v44` и сохраняет поддержку более ранних версий. Миграция `v44 → v45` добавляет отсутствующий `upperZoneAutoScrollEnabled=true`; canonical preset/template также включают upward-follow. Миграция `v43 → v44` по-прежнему добавляет `sceneTwoOverflowYVisible=false`.
- Категория единственной руки называется «Рука» и содержит `handVisibilityMode` (`always|hover|hidden`, default `always`), `handImageChangeDelayMs` (`0–1000`, integer, default `0`) и `rockGrabRadiusVh`. Категория «Камера» содержит `cameraFollowLerp` (`0.01–1`, default `0.1`), `cameraFollowDownEnabled` (default `true`), `upperZoneAutoScrollEnabled` (default `true`) и `sceneTwoOverflowYVisible` (default `false`). Последний параметр ставит inline `overflow-y: hidden|auto` на `<html>`; программный `window.scrollTo` и `cameraFollowDirectionalScrollY()` продолжают работать при `hidden`. «Препятствия → Окна» содержит девять versioned controls и `WindowObstaclePermissionControl` со статусом и test action.
...
- `scene.css` регистрирует `@font-face` `Comico` из локальных `Comico-Regular.woff2`/`.woff`; семейство назначено только `.title` и `.title2`. JSX и document title используют точные строки `The Path of Tzarey` и `miniature`, а `text-transform` не меняет их регистр.
...
- `settings.update` использует schema `45` и optimistic `settingsRevision`.
