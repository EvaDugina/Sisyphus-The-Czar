...
- В группе «Камень» находятся независимые select-контролы основного и fold-изображения, отдельные проценты уменьшения при нажатии и пульсе, `rockWallPenetrationPercent`, настройки автоматического прыжка и три hop-контрола: `preclickHopGuardClickCount`, `preclickHopActivationRadiusPercent` и `preclickHopMaxDistancePercent`. Их диапазоны — `0–10`, `0–300` и `0–150`; `rockPulseShrinkPercent` использует `enabledWhen: "rockPulseEnabled"`.
...
- До настоящего mouse-захвата `preclickRockGuidance.completed=false`: физика остаётся suspended, hop активен, `html/body.is-manual-scroll-disabled` блокируют ручной vertical scroll. `consumePreclickGuardClick()` перехватывает первые `preclickHopGuardClickCount` валидных pointerdown после проверки фазы и window-obstacle, увеличивает только `guardClicksUsed` и запускает hop до `completePreclickRockGuidance()`/`beginSharedDrag()`. Автоматические входы счётчик не меняют. Клик N+1 проходит через прежний `startDrag`, материализует wrapped-центр и включает физику/camera-follow. Инициализация и restart используют `resetPreclickRockGuidance()`, который обнуляет счётчик.
...
- UI/Fold smoke проверяет миграцию `v39 → v40` из старых `...Vw`-ключей в `1 / 0–300 / 0–150`, удаление legacy и наличие ровно трёх hop-контролов с процентным output.
...
- Постоянный preclick-hop smoke проверяет guidance, scroll lock, reload/restart, радиус от текущей ширины камня, направленный максимум, безопасный X/Y/corner wrap, resize, reduced motion, смех и кликабельность. Отдельный сценарий задаёт N=3, доказывает, что auto-hop не расходует счётчик и не отправляет `control.acquire`, первые три клика только отталкивают камень, четвёртый материализует позицию и включает физику, а restart обнуляет счётчик.
