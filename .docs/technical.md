...
2. Каждый корневой модуль передаёт собственный `sceneId` в общий `ScenePage`/`createSisyphusRuntime`. Runtime вызывает `POST /api/sessions` с scene-specific initial state, не восстанавливает состояние другой страницы и загружает параметры только из ключа `sisyphus-czar-settings-v51:<scene-id>`.
...
11. `SettingsPanel` монтируется inline внутри текущего `ScenePage`. `settingsGroupsForScene(sceneId)` создаёт новые control-объекты с `ownerSceneId`; controller использует scene-specific settings/version/import keys. `scrollToSceneStart()` открывает сцены 1/2 у нижнего экрана, а сцену 3 — у вершины.
...
- `settings.mjs` декларативно определяет допустимость шаблонов контролов для каждой страницы, а `settingsGroupsForScene()` материализует независимые экземпляры с единственным `ownerSceneId`. В DOM нет switcher и контролов чужой сцены; одинаковые имена на разных страницах не означают общее значение благодаря разным runtime, session namespace и localStorage keys. Scene 1 владеет preclick-настройками, scene 2 — подъёмом/препятствиями/траекторией, scene 3 — финальным падением, дождём и секундомером.
...
- Scene-pages smoke проверяет inline UI без switcher, отсутствие чужих контролов, независимое восстановление одинакового визуального параметра, завершение сцены 2 на отпечатке и завершение сцены 3 только после падения, нижнего camera-follow и дождя.
...
- Unit/integration regression проверяет scene route/nextPath, отдельные ключи параметров, версий и session namespace, принадлежность materialized control единственной сцене, сообщения production preset, полную замену canonical preset и серверные physics/room-settings contracts.
