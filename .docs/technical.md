...
- Постоянный preclick-hop smoke проверяет guidance, scroll lock, reload/restart, радиус от текущей ширины камня, два обязательных radius-hop со смехом, третий forced miss, поведение `0%`, скорость/кривую, дискретный эхо-след, отложенный popup с картинами `01–03`, отсутствие auto-close, отсутствие обычной траектории в сцене 1, направленный максимум, безопасный X/Y/corner wrap, resize, reduced motion и кликабельность. Отдельный сценарий задаёт N=3 и меняет запрошенную ширину между кликами: auto-hop не расходует счётчик и не отправляет `control.acquire`, первые три клика не воспроизводят gachi-звук, последовательно открывают картины `01`, `02`, `03`, все старые и два финальных окна сохраняют размер первого popup, а четвёртый клик фокусирует живые окна и завершает сцену с причиной `first-real-rock-press`.
