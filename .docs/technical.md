...
2. Каждый корневой модуль передаёт собственный `sceneId` в общий `ScenePage`/`createSisyphusRuntime`. Runtime вызывает `POST /api/sessions` с scene-specific initial state, не восстанавливает состояние другой страницы и загружает параметры только из ключа `sisyphus-czar-settings-v52:<scene-id>`. `SessionManager` нормализует, хранит, сериализует и возвращает `sceneId`; server-side стекло/барьер/height gates разрешены только для `turnip`, hold-таймеры — для `turnip|juices`, а секундомер/final fall — только для `juices`.
...
- Shared room settings schema и settings schema — `52`, localStorage настроек — `sisyphus-czar-settings-v52`; цепочка legacy-ключей начинается с `v51` и сохраняет поддержку более ранних версий. Миграция `v51 → v52` вычисляет `preclickPopupWidthViewportFraction = rockActivatedWidthVw / 100 × preclickPopupSizeMultiplier`, ограничивает результат диапазоном `0.01–1`, удаляет legacy-множитель и добавляет defaults эхо-следа.
...
- В группе «Камень» находятся независимые select-контролы основного и fold-изображения, отдельные проценты уменьшения при нажатии и пульсе, `rockWallPenetrationPercent`, настройки автоматического прыжка и десять scene-1 контролов: `preclickHopGuardClickCount`, `preclickPopupDelayMs`, `preclickPopupWidthViewportFraction`, `birchBackgroundEnabled`, `birchScalePercent`, `preclickHopActivationRadiusPercent`, `preclickHopMaxDistancePercent`, `preclickHopMissProbabilityPercent`, `preclickHopSpeedPxPerSecond` и `preclickHopSpeedEasing`. Ширина popup санитизируется как finite `0.01–1` с default `0.2` и шагом UI `0.01`; высота не имеет отдельного параметра. Масштаб берёз — integer `100–400` с шагом UI `10`; `birchScalePercent` использует `enabledWhen: "birchBackgroundEnabled"`. Три per-scene контрола масштаба расположены как `rockMinWidthVw → rockActivatedWidthVw → rockMaxWidthVw`; `rockPulseShrinkPercent` использует `enabledWhen: "rockPulseEnabled"`.
...
- `settings.mjs` декларативно определяет допустимость шаблонов контролов для каждой страницы, а `settingsGroupsForScene()` материализует независимые экземпляры с единственным `ownerSceneId`. Актуальная матрица содержит `46 / 103 / 104` контрола. Scene 1 владеет preclick/берёзами/эхо-следом; scene 2 — overflow, height gates, барьером, окнами и стеклом; scene 3 — final fall, дождём и секундомером. Камера вверх/вниз, hold/auto-release, траектория и морось материализуются отдельно в сценах 2 и 3; визуальные параметры и звук руки — отдельно во всех трёх. Runtime дополнительно ограждает foreign birch/echo/overflow/glass/gate/barrier/rain effects, поэтому старый localStorage или preset не активирует скрытую настройку.
...
- `settings.update` использует schema `52` и optimistic `settingsRevision`.
...
- Scene-pages smoke проверяет точные `46 / 103 / 104` контрола, отсутствие чужого клиентского влияния, независимое восстановление одинакового визуального параметра, миграцию scene-1 storage `v51 → v52`, desktop resize, отдельный wheel-scroll без изменения `window.scrollY`, отсутствие обрезания `5%`, narrow viewport и завершение сцен 2/3.
