...
- Shared room settings schema и settings schema — `41`, localStorage — `sisyphus-czar-settings-v41`; ключ `v40` читается как legacy. Миграция `v40 → v41` переводит `trailUnlimited=true` в `trailMaxPoints=10 000`, сохраняет остальные значения в диапазоне `20–10 000` и удаляет legacy-поле.
...
- `trailRenderProfile`, `glowOptimizationMode`, `glowTargetFps`, `glowBufferScalePercent`, `glowUpdateFps`, `glowMaxPoints` и `glowDecimation` имеют `scope: "local"`: сохраняются в `v41`, но фильтруются из version snapshots, server templates и broadcast.
...
- `settings.update` использует schema `41` и optimistic `settingsRevision`.
...
- UI/Fold smoke проверяет миграцию `v40 → v41`, удаление `trailUnlimited`, максимум 10 000, локальный render-профиль, две сценовые кнопки и общий раздел «Траектория».
