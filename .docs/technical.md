...
- Shared room settings schema — `36`, settings schema — `38`, localStorage — `sisyphus-czar-settings-v38`. `rockImageId` и `foldRockImageId` принимают только `rock-03`, `rock` или `rock2`; fallback — `rock-03`. `rockPulseShrinkPercent` имеет диапазон `0–50`, default `5` и при миграции schema `<35` наследует прежний `rockPressShrinkPercent`. Миграция `<34/<36` переносит legacy parallax в hop-контракт, schema `<37` преобразует `handAlwaysVisible=true/false` в `handVisibilityMode=always/hover`, а schema `<38` добавляет `foldPositionPercent=0` и наследует `foldPanelHeightVh` из прежнего `foldZoneSize`.
...
- В группе «Камень» находятся независимые select-контролы основного и fold-изображения, отдельные проценты уменьшения при нажатии и пульсе, настройки автоматического прыжка и ровно два hop-контрола: `preclickHopActivationRadiusVw` и `preclickHopMaxDistanceVw`. `rockPulseShrinkPercent` использует `enabledWhen: "rockPulseEnabled"`.
...
- `glowOptimizationMode`, `glowTargetFps`, `glowBufferScalePercent`, `glowUpdateFps`, `glowMaxPoints` и `glowDecimation` имеют `scope: "local"`: сохраняются в v36, но фильтруются из version snapshots, server templates и broadcast.
...
- Каждый новый вход мыши в `preclickHopActivationRadiusVw` выполняет один накопительный hop от текущего визуального центра. Скорость входа `0–2000 px/s` линейно отображается в `0.28–1.0 × preclickHopMaxDistanceVw`. Runtime за `400 ms` применяет `cubic-bezier(0.22, 1, 0.36, 1)` покадрово, каждый кадр нормализует центр по модулю `innerWidth/innerHeight` и сохраняет overshoot. Reduced motion сразу применяет конечный wrapped-центр; resize отменяет активный frame и нормализует текущий центр. Первый захват материализует wrapped-центр в physical/server coordinates. Каждый hop создаёт независимый `Audio` из `assets/audio/Смех.mp3`; dispose/restart очищают animation и оставшиеся элементы.
- До первого допустимого mouse-захвата `preclickRockGuidance.completed=false`: физика остаётся suspended, hop активен, `html/body.is-manual-scroll-disabled` блокируют колесо и ручной vertical scroll. Прямой `pointerdown` по камню или круг курсора радиуса `rockGrabRadiusVh / 100 · innerHeight`, пересекающий `getBoundingClientRect()` камня, проходит через единый `startDrag`. Успешный захват завершает guidance, обнуляет CSS hop-offset после materialize, включает физику и camera-follow. Начальная загрузка и `resetLocalExperience()` используют единый `resetPreclickRockGuidance()`.
...
- UI/Fold smoke проверяет миграции `v32 draftFold* → v36 fold*`, `v35 → v36` для canonical hop-радиуса и удаления legacy, `v34 → v36` для изображений/pulse shrink, а также наличие ровно двух hop-контролов.
- Dev smoke проверяет объединённое сохранение именованной версии и комнаты, отсутствие дубликатов в общем каталоге и миграцию legacy-настроек в v36, включая px→vw, Fold, SVG-курсор и visual rock settings.
- Постоянный preclick-hop smoke проверяет guidance, scroll lock, reload/restart, один hop на вход, отсутствие повторов внутри радиуса, `28–100%` максимума, накопление, X/Y/corner wrap с overshoot, resize, reduced motion, смех, кликабельность, materialize первого захвата и camera-follow.
