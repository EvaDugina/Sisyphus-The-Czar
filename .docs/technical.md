...
- Scene-pages smoke проверяет точные `38 / 103 / 106` контролов, режим и условный select Gogh, отсутствие скрытых scene-1 полей и чужого клиентского влияния, независимое восстановление одинакового визуального параметра, миграцию scene-1 storage, desktop resize, отдельный wheel-scroll без изменения `window.scrollY`, отсутствие обрезания значений, narrow viewport и завершение сцен 2/3.
