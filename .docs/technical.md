...
- `roomSettings`, `roomSettingsVersion=37`, `settingsRevision`;
...
- Shared room settings schema — `38`, settings schema — `40`, localStorage — `sisyphus-czar-settings-v40`; ключ `v39` читается как legacy. `migratePreclickHopSettings()` переносит числовые `preclickHopActivationRadiusVw` и `preclickHopMaxDistanceVw` в процентные поля, ограничивает максимум до `150` и добавляет default одного фейкового клика. `rockWallPenetrationPercent` сохраняет диапазон `0–50` и default `20`.
- Категория единственной руки называется «Рука» и содержит `handVisibilityMode` (`always|hover|hidden`, default `always`), `handImageChangeDelayMs` (`0–1000`, integer, default `0`) и `rockGrabRadiusVh`. Категория «Камера» содержит `cameraFollowLerp` (`0.01–1`, default `0.1`). «Препятствия → Окна» содержит девять versioned controls и `WindowObstaclePermissionControl` со статусом и test action.
- В группе «Камень» находятся независимые select-контролы основного и fold-изображения, отдельные проценты уменьшения при нажатии и пульсе, `rockWallPenetrationPercent`, настройки автоматического прыжка и три hop-контрола: `preclickHopGuardClickCount` с UI-меткой «Количество фейковых кликов», `preclickHopActivationRadiusPercent` и `preclickHopMaxDistancePercent`. Их диапазоны — `0–10`, `0–300` и `0–150`; `rockPulseShrinkPercent` использует `enabledWhen: "rockPulseEnabled"`.
...
- `glowOptimizationMode`, `glowTargetFps`, `glowBufferScalePercent`, `glowUpdateFps`, `glowMaxPoints` и `glowDecimation` имеют `scope: "local"`: сохраняются в `v40`, но фильтруются из version snapshots, server templates и broadcast.
...
- Каждый новый вход мыши в радиус `preclickHopActivationRadiusPercent / 100 × getBoundingClientRect().width` выполняет один накопительный hop от текущего визуального центра. `preclickDirectionalViewportSpan()` выбирает единицу пути по вектору и пропорциям viewport; скорость `0–2000 px/s` линейно отображается в `0.28–1.0` от UI-максимума. Перед анимацией `preclickHopPathIsSafe()` непрерывно проверяет отрезок против повторяющихся на торе копий зоны руки, а `preclickToroidalDistance()` исключает визуальный возврат в старт. Кандидаты перебираются по ближайшим углам и уменьшениям дистанции, не превышая UI cap. Покадровый wrap, `400 ms`, easing, reduced motion, resize и отдельный `Смех.mp3` сохраняются.
- До настоящего mouse-захвата `preclickRockGuidance.completed=false`: физика остаётся suspended, hop активен, `html/body.is-manual-scroll-disabled` блокируют ручной vertical scroll. `consumePreclickGuardClick()` перехватывает первые `preclickHopGuardClickCount` валидных pointerdown после проверки фазы и window-obstacle, увеличивает только `guardClicksUsed` и запускает hop до создания gachi, `completePreclickRockGuidance()` и `beginSharedDrag()`. Автоматические входы счётчик не меняют. Клик N+1 проходит через `playGachiClickSound()` и прежний `startDrag`, материализует wrapped-центр и включает физику/camera-follow. Инициализация и restart используют `resetPreclickRockGuidance()`, который обнуляет счётчик.
...
- `FoldLayer` входит в основной `<App />`, читает живой `params` runtime через ref и синхронизирует неинтерактивную копию сцены, canvas-следа и дождя. Слой имеет `position:absolute` внутри относительного `#root`, полную ширину и высоту `foldPanelHeightVh`; fixed/sticky режим не используется. Для trail-canvas копирование выполняется только при изменении `data-canvas-revision`; rain-canvas сохраняют покадровую синхронизацию.
...
- Физическая и preclick-сцены используют единые `trail.points`, canonical history, ограничения длины, anchor, стили и очистку. Физическая ветка вычисляет anchor из `motion`, а preclick-hop — из уже рассчитанного wrapped-центра и сохранённого `worldRect`; обе передают точку в `recordTrailAnchorPoint()`.
...
- UI/Fold smoke проверяет миграцию `v39 → v40` из старых `...Vw`-ключей в `1 / 0–300 / 0–150`, удаление legacy, наличие ровно трёх hop-контролов с процентным output, две сценовые кнопки, их `aria-pressed`, скрытие нерелевантных контролов и целых разделов, а также общий раздел «Траектория», его стиль, очистку и одно несбрасываемое значение при переключении.
- Dev smoke проверяет сохранение всех трёх hop-параметров в `v40` вместе с остальными UI-настройками, объединённое сохранение именованной версии и комнаты и миграции более старых snapshots.
- Постоянный preclick-hop smoke проверяет guidance, scroll lock, reload/restart, радиус от текущей ширины камня, направленный максимум, безопасный X/Y/corner wrap, resize, reduced motion, смех и кликабельность. Отдельные сценарии доказывают создание нескольких canonical/local trail-точек до завершения первой сцены и задают N=3: auto-hop не расходует счётчик и не отправляет `control.acquire`, первые три клика только отталкивают камень без gachi, четвёртый создаёт один gachi, материализует позицию и включает физику, а restart обнуляет счётчик.
