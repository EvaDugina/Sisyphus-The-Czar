...
- Серверная settings schema — `30`; localStorage key — `sisyphus-czar-settings-v30`, `v29` и более ранние ключи мигрируются как legacy.
- Shared room settings schema — `26`. `preclickParallaxActivationRadiusVw` имеет диапазон `0–200` и default `50`; `preclickParallaxMaxOffsetVw` — диапазон `0–150` и default `0.6`; `preclickParallaxStartDelayMs` — `0–1000`, default `0`; `rockGrabRadiusVh` — `0–10`, default `0`. Старые `preclickParallaxActivationRadiusPx` и `preclickParallaxMaxOffsetPx` мигрируют по фиксированному соотношению `20 px = 1 vw`; payload версий `<26` получают нулевые defaults новых полей.
...
- В группе «Камень» находятся два checkbox, три range-контрола автоматического прыжка и шесть parallax-контролов: максимум отклонения, радиус активации, задержка запуска, toggle-кнопка инверсии, длительность возврата и cubic-bezier-редактор с графиком.
