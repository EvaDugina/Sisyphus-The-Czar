...
- Scene-2 scale lifecycle хранит `sceneTwoSizeState` и `sceneTwoSizeCycleArmed`. Успешный захват взводит новый цикл и вызывает `beginSceneTwoGrabScale()`, останавливая пульс; освобождение переводит камень в `airborne` и возобновляет пульс, а `settleSceneTwoRockScaleOnGround()` на первом ground-touch ставит `ground`, снимает флаг и сохраняет пульс активным. Повторные отскоки при снятом флаге не взводят size-cycle; следующий цикл начинается только с нового успешного захвата. `SceneTwoPage` наблюдает снятие runtime-класса `is-dragging` и сбрасывает локальную sticky-защёлку после фактического release, не пропуская обычные pointerup/pointercancel во время удержания. `gachiClickSoundFilename="none"` проходит shared sanitization и завершает `playGachiClickSound()` до создания `Audio`. Remote snapshot использует ту же state-machine; camera-follow независимо применяет пары up/down enabled+lerp.
