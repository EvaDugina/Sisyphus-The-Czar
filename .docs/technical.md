...
2. Каждый корневой модуль передаёт собственный `sceneId` в общий `ScenePage`/`createSisyphusRuntime`. Runtime вызывает `POST /api/sessions` с scene-specific initial state, не восстанавливает состояние другой страницы и загружает параметры только из ключа `sisyphus-czar-settings-v51:<scene-id>`. `SessionManager` нормализует, хранит, сериализует и возвращает `sceneId`; server-side стекло/барьер/height gates разрешены только для `turnip`, hold-таймеры — для `turnip|juices`, а секундомер/final fall — только для `juices`.
...
- Scene-pages smoke проверяет точные `41 / 103 / 104` контрола, отсутствие чужого клиентского влияния, независимое восстановление одинакового визуального параметра, desktop resize, отдельный wheel-scroll без изменения `window.scrollY`, отсутствие обрезания `5%`, narrow viewport и завершение сцен 2/3.
