...
- Shared room settings schema — `35`, settings schema — `37`, localStorage — `sisyphus-czar-settings-v37`. `rockImageId` и `foldRockImageId` принимают только `rock-03`, `rock` или `rock2`; fallback — `rock-03`. `rockPulseShrinkPercent` имеет диапазон `0–50`, default `5` и при миграции schema `<35` наследует прежний `rockPressShrinkPercent`. Миграция `<34/<36` переносит legacy parallax в hop-контракт, а schema `<37` преобразует `handAlwaysVisible=true/false` в `handVisibilityMode=always/hover`, добавляет `handImageChangeDelayMs=0` и удаляет boolean-ключ.
