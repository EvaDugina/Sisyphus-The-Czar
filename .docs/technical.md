...
2. Каждый корневой модуль передаёт собственный `sceneId` в общий `ScenePage`/`createSisyphusRuntime`. Runtime вызывает `POST /api/sessions` с scene-specific initial state, не восстанавливает состояние другой страницы и загружает параметры только из ключа `sisyphus-czar-settings-v53:<scene-id>`. `SessionManager` нормализует, хранит, сериализует и возвращает `sceneId`; server-side стекло/барьер/height gates разрешены только для `turnip`, hold-таймеры — для `turnip|juices`, а секундомер/final fall — только для `juices`.
...
- Shared room settings schema и settings schema — `53`, localStorage настроек — `sisyphus-czar-settings-v53`; цепочка legacy-ключей начинается с `v52` и сохраняет поддержку более ранних версий. Миграция `v52 → v53` добавляет `preclickPopupArtworkMode="shuffle"` и `preclickPopupArtworkId="01.png"`; предыдущая миграция `v51 → v52` по-прежнему переводит legacy-множитель popup в viewport-долю `0.01–1` и добавляет defaults эхо-следа.
