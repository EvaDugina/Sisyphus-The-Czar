...
8. Runtime выбирает click-gachi только из совпадения shared-манифеста с Vite audio glob, останавливает единственный активный канал при release/переходе в `falling`, а `СимуляцияОргазма.mov` запускает по серверному `groundTouchSeq` или локальному `touchedGround`.
...
- Collaboration smoke проверяет случайный актуальный gachi по click/tap, остановку его при начале падения, отдельный `СимуляцияОргазма.mov` на каждый новый ground impact и отсутствие повторного запуска на кадрах покоя.
