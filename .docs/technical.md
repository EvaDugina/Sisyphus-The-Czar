...
- Shared room settings schema — `37`, settings schema — `39`, localStorage — `sisyphus-czar-settings-v39`; ключ `v38` читается как legacy. `rockWallPenetrationPercent` имеет диапазон `0–50` и default `20`; отсутствующее в старом snapshot значение получает default через sanitizer. Остальные миграции сохраняют прежние правила для изображений, pulse, hop, руки и Fold.
...
- В группе «Камень» находятся независимые select-контролы основного и fold-изображения, отдельные проценты уменьшения при нажатии и пульсе, `rockWallPenetrationPercent`, настройки автоматического прыжка и ровно два hop-контрола: `preclickHopActivationRadiusVw` и `preclickHopMaxDistanceVw`. `rockPulseShrinkPercent` использует `enabledWhen: "rockPulseEnabled"`.
...
- `glowOptimizationMode`, `glowTargetFps`, `glowBufferScalePercent`, `glowUpdateFps`, `glowMaxPoints` и `glowDecimation` имеют `scope: "local"`: сохраняются в `v39`, но фильтруются из version snapshots, server templates и broadcast.
