...
- `ROOM_SETTINGS_VERSION=30`, `SETTINGS_SCHEMA_VERSION=32`, текущий localStorage-ключ — `sisyphus-czar-settings-v32`; `v31` читается как legacy. Миграция добавляет `customCursorEnabled=false` и `customCursorSizePx=32`, санитайзер ограничивает размер диапазоном `8–128`.
...
- `glowOptimizationMode`, `glowTargetFps`, `glowBufferScalePercent`, `glowUpdateFps`, `glowMaxPoints` и `glowDecimation` имеют `scope: "local"`: сохраняются в v32, но фильтруются из version snapshots, server templates и broadcast.
...
- Draft smoke проверяет единственную compact-кнопку сохранения, применение room settings и отсутствие отдельного `.settings-room-save`, SVG `handopen/handgrabbing` и размер для локальной/remote-руки, идентичность основной Fold-сцены и настроек на `/` и `/drafts/`, диапазон максимума parallax в `vw`, режим постоянной/hover-руки, сохранение preclick и настроек при повторных reload до первого захвата, видимость пользовательской кнопки restart на игровой странице, camera lerp, obstacle defaults, popup-blocked UI, сохранение glow-профиля, select-зависимости и безопасную структуру зеркала.
- Dev smoke проверяет объединённое сохранение именованной версии и комнаты, отсутствие дубликатов в общем каталоге, миграцию legacy-настроек в v32, включая перевод parallax-радиуса и максимума из px в vw, defaults SVG-курсора `off / 32 px`, defaults динамического перехода, default-off для инверсии, default-on для постоянной руки, лимит glow-точек, отсутствие проходов при `glow=0` и копирование Fold только при новой canvas revision.
