...
- Shared room settings schema и settings schema — `49`, localStorage — `sisyphus-czar-settings-v49`; цепочка legacy-ключей начинается с `v48` и сохраняет поддержку более ранних версий. Миграция `v47 → v48` добавляет `preclickPopupSizeMultiplier=2`, `birchBackgroundEnabled=false` и `birchScalePercent=100`. Миграция `v48 → v49` копирует legacy `cameraFollowLerp` в `cameraFollowUpLerp` и `cameraFollowDownLerp`, сохраняет `cameraFollowDownEnabled`, добавляет `cameraFollowUpEnabled=true`, удаляет `upperZoneAutoScrollEnabled` и принудительно ставит `rockAccelerationEnabled=false`; старый автоскролл не включает ни слежение вверх, ни ускорение.
...
- После активации физики `restoreRockInitialScaleOnSceneTwoGrab()` вызывает `rockSceneTwoGrabScaleFactor()`: если текущий activation-factor меньше `1`, захват снизу возвращает его к `1` с существующим `is-activation-scaling` transition `300 ms`. Кадр camera-follow сохраняет anchor `0.5`, вычисляет цель по центру камня и передаёт текущую и целевую позиции в `cameraFollowDirectionalScrollY()`. При цели выше текущего viewport используются `cameraFollowUpEnabled` и `cameraFollowUpLerp`, при цели ниже — `cameraFollowDownEnabled` и `cameraFollowDownLerp`; выключенный тумблер блокирует только своё направление. Немедленное центрирование восстановленной активной сессии выполняется отдельным служебным путём и не зависит от этих тумблеров.
