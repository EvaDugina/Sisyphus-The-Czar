...
- UI/Fold smoke проверяет миграцию `v39 → v40` из старых `...Vw`-ключей в `1 / 0–300 / 0–150`, удаление legacy, наличие ровно трёх hop-контролов с процентным output, две сценовые кнопки, их `aria-pressed`, скрытие нерелевантных контролов и целых разделов, а также сохранение несохранённого shared-значения при переключении.
...
- Постоянный preclick-hop smoke проверяет guidance, scroll lock, reload/restart, радиус от текущей ширины камня, направленный максимум, безопасный X/Y/corner wrap, resize, reduced motion, смех и кликабельность. Отдельный сценарий задаёт N=3, доказывает, что auto-hop не расходует счётчик и не отправляет `control.acquire`, первые три клика только отталкивают камень без gachi, четвёртый создаёт один gachi, материализует позицию и включает физику, а restart обнуляет счётчик.
