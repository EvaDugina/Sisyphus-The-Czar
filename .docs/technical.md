...
- Shared room settings schema и settings schema — `46`, localStorage — `sisyphus-czar-settings-v46`; цепочка legacy-ключей начинается с `v45` и сохраняет поддержку более ранних версий. Миграция `v45 → v46` добавляет `preclickPopupDelayMs=200`; миграция `v44 → v45` по-прежнему добавляет отсутствующий `upperZoneAutoScrollEnabled=true`.
...
- В группе «Камень» находятся независимые select-контролы основного и fold-изображения, отдельные проценты уменьшения при нажатии и пульсе, `rockWallPenetrationPercent`, настройки автоматического прыжка и семь scene-1 контролов: `preclickHopGuardClickCount`, `preclickPopupDelayMs`, `preclickHopActivationRadiusPercent`, `preclickHopMaxDistancePercent`, `preclickHopMissProbabilityPercent`, `preclickHopSpeedPxPerSecond` и `preclickHopSpeedEasing`. Числовые диапазоны включают задержку popup `0–1000 ms`; easing проходит общий cubic-bezier sanitizer. Три scene-2 контрола масштаба расположены как `rockMinWidthVw → rockActivatedWidthVw → rockMaxWidthVw`; `rockPulseShrinkPercent` использует `enabledWhen: "rockPulseEnabled"`.
...
- `settings.update` использует schema `46` и optimistic `settingsRevision`.
...
- Постоянный preclick-hop smoke проверяет guidance, scroll lock, reload/restart, радиус от текущей ширины камня, два обязательных radius-hop со смехом, третий forced miss, поведение `0%`, скорость/кривую, отложенный popup с `rock.webp` шириной из `rockActivatedWidthVw` и natural aspect ratio, отсутствие auto-close, отсутствие trail в сцене 1, направленный максимум, безопасный X/Y/corner wrap, resize, reduced motion и кликабельность. Отдельные сценарии задают N=3: auto-hop не расходует счётчик и не отправляет `control.acquire`, первые три клика воспроизводят `Camen.mp3`, открывают image-popup после задержки и отталкивают камень со смехом, четвёртый бесшумно материализует позицию и включает физику, а три следующих клика сцены 2 оставляют одновременно активными три экземпляра выбранного gachi без stop.
