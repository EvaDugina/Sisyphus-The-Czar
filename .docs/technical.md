...
8. Runtime сначала передаёт preclick-нажатие в `consumePreclickGuardClick`: если это один из первых N фейковых кликов, выполняется hop с единственным `Смех.mp3`, а click-gachi не создаётся. Каждый кадр preclick-hop передаёт рассчитанную визуальную anchor-точку в общий trail-recorder и ставит session/glow invalidation в единый frame-coordinator; history до checkpoint не перестраивается. Геометрия мира считывается один раз перед анимацией, без принудительного покадрового layout. Активационный клик N+1 проходит без аудио; последующий primary click/tap сцены 2 выбирает санитизированный `gachiClickSoundFilename` из совпадения shared-манифеста с Vite audio glob, создаёт независимый `Audio` и при переходе в `FALLING` останавливает активные экземпляры. Успешный новый захват выставляет одноразовый `groundImpactAudio.armed`; первый следующий локальный `touchedGround` или рост серверного `groundTouchSeq` погашает флаг и запускает `СимуляцияОргазма.mov`. Последующие отскоки и sequence jumps без нового захвата не воспроизводят звук; restart и dispose сбрасывают разрешение.
...
- `roomSettings`, `roomSettingsVersion=41`, `settingsRevision`;
...
- Shared room settings schema и settings schema — `43`, localStorage — `sisyphus-czar-settings-v43`; цепочка legacy-ключей начинается с `v42` и сохраняет поддержку более ранних версий. Миграция `v42 → v43` добавляет `cameraFollowDownEnabled=true`, `upperZoneAutoScrollEnabled=false` и `gachiClickSoundFilename=Camen.mp3`. Миграция `v41 → v42` по-прежнему добавляет параметры вероятности, скорости и easing preclick-hop.
- Категория единственной руки называется «Рука» и содержит `handVisibilityMode` (`always|hover|hidden`, default `always`), `handImageChangeDelayMs` (`0–1000`, integer, default `0`) и `rockGrabRadiusVh`. Категория «Камера» содержит `cameraFollowLerp` (`0.01–1`, default `0.1`), `cameraFollowDownEnabled` (default `true`) и `upperZoneAutoScrollEnabled` (default `false`). `cameraFollowDirectionalScrollY()` применяет lerp только для разрешённого направления. «Препятствия → Окна» содержит девять versioned controls и `WindowObstaclePermissionControl` со статусом и test action.
- В группе «Камень» находятся независимые select-контролы основного и fold-изображения, отдельные проценты уменьшения при нажатии и пульсе, `rockWallPenetrationPercent`, настройки автоматического прыжка и шесть scene-1 hop-контролов: `preclickHopGuardClickCount`, `preclickHopActivationRadiusPercent`, `preclickHopMaxDistancePercent`, `preclickHopMissProbabilityPercent`, `preclickHopSpeedPxPerSecond` и `preclickHopSpeedEasing`. Их числовые диапазоны — `0–10`, `0–300`, `0–150`, `0–100` и `100–5000`; easing проходит общий cubic-bezier sanitizer. Три scene-2 контрола масштаба расположены как `rockMinWidthVw → rockActivatedWidthVw → rockMaxWidthVw`; `rockPulseShrinkPercent` использует `enabledWhen: "rockPulseEnabled"`.
- `settings.mjs` экспортирует две сцены и декларативно классифицирует контролы как scene-1-only, scene-2-only или shared. `SettingsPanel` выводит две верхние кнопки с `aria-pressed`, а при переключении меняет `hidden` на контролах, подгруппах и пустых группах. DOM-узлы input не размонтируются, поэтому единые значения, dirty-state и зависимости контроллера сохраняются.
...
- `trailRenderProfile`, `glowOptimizationMode`, `glowTargetFps`, `glowBufferScalePercent`, `glowUpdateFps`, `glowMaxPoints` и `glowDecimation` имеют `scope: "local"`: сохраняются в `v43`, но фильтруются из version snapshots, server templates и broadcast.
...
- Каждый вход мыши в радиус `preclickHopActivationRadiusPercent / 100 × getBoundingClientRect().width` проходит через `preclickRadiusHopDecision()`: первые два успешных radius-hop обязательны, следующая попытка получает единственный `forced-miss`, затем решение использует `preclickHopMissProbabilityPercent / 100`. Выход из радиуса взводит следующую проверку; restart сбрасывает `radiusHopCount` и `forcedRadiusMissConsumed`. `preclickDirectionalViewportSpan()` выбирает единицу пути по вектору и пропорциям viewport, а скорость указателя `0–2000 px/s` линейно отображается в `0.28–1.0` от UI-максимума. Перед анимацией `preclickHopPathIsSafe()` непрерывно проверяет отрезок против повторяющихся на торе копий зоны руки, а `preclickToroidalDistance()` исключает визуальный возврат в старт. Кандидаты перебираются по ближайшим углам и уменьшениям дистанции, не превышая UI cap. Длительность вычисляет `preclickHopDurationMs(pathLength, preclickHopSpeedPxPerSecond)`, прогресс использует `preclickHopSpeedEasing`; покадровый wrap, reduced motion и resize сохраняются, а автоматический radius-hop не вызывает аудио.
- До настоящего mouse-захвата `preclickRockGuidance.completed=false`: физика остаётся suspended, hop активен, `html/body.is-manual-scroll-disabled` блокируют ручной vertical scroll. `consumePreclickGuardClick()` перехватывает первые `preclickHopGuardClickCount` валидных pointerdown после проверки фазы и obstacle-window, увеличивает только `guardClicksUsed`, вызывает `openPreclickImageWindow()`, один раз запускает `Смех.mp3` и выполняет hop до `completePreclickRockGuidance()` и `beginSharedDrag()`. Popup шириной до `640 px` вычисляет высоту по aspect ratio `ВЗГЛЯД.jpg`, центрируется относительно screen-position pointer с ограничением рабочей областью, заполняется `<img>` на `100vw × 100vh`, закрывается кликом или через `2 s` и не входит в `activeObstacleCount()`. Автоматические входы click-счётчик не меняют и аудио не запускают. Клик N+1 без смеха и gachi материализует wrapped-центр и включает физику/camera-follow; последующие клики сцены 2 вызывают `playGachiClickSound()` с санитизированным `gachiClickSoundFilename`.
- После активации физики `restoreRockInitialScaleOnSceneTwoGrab()` вызывает `rockSceneTwoGrabScaleFactor()`: если текущий activation-factor меньше `1`, захват снизу возвращает его к `1` с существующим `is-activation-scaling` transition `300 ms`. Кадр camera-follow передаёт текущую и целевую позиции в `cameraFollowDirectionalScrollY()`: движение вниз разрешает `cameraFollowDownEnabled`, вверх — независимый `upperZoneAutoScrollEnabled`; выключенный флаг немедленно оставляет текущий `scrollY` без дальнейшей интерполяции.
...
- `settings.update` использует schema `43` и optimistic `settingsRevision`.
...
- UI/Fold smoke проверяет legacy-миграции в `v43`, удаление `trailUnlimited`, максимум 10 000, hop-defaults, три новых параметра сцены 2, локальный render-профиль, две сценовые кнопки, scene-specific размеры и общий раздел «Траектория».
...
- Постоянный preclick-hop smoke проверяет guidance, scroll lock, reload/restart, радиус от текущей ширины камня, два обязательных radius-hop без смеха, третий forced miss, поведение `0%`, скорость/кривую, центрированное окно с `ВЗГЛЯД.jpg`, направленный максимум, безопасный X/Y/corner wrap, resize, reduced motion и кликабельность. Отдельные сценарии задают N=3: auto-hop не расходует счётчик и не отправляет `control.acquire`, первые три клика открывают popup, воспроизводят по одному смеху и отталкивают камень без gachi, четвёртый бесшумно материализует позицию и включает физику, а следующий клик сцены 2 создаёт выбранный `Camen.mp3`.
...
- Unit-тест контроллера использует виртуальные timeout/interval и fake popup: проверяет отсутствие дублирующего schedule, одновременные obstacle-окна, независимый click/2s close, выход/возврат в диапазон, паузу при блокировке, а также геометрию и неблокирующий lifecycle preclick image-window.
