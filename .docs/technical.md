...
8. Runtime выбирает click-gachi только из совпадения shared-манифеста с Vite audio glob, создаёт независимый `Audio` для каждого primary click/tap и при переходе в `FALLING` останавливает весь набор активных экземпляров. `СимуляцияОргазма.mov` не запускается началом падения: каждый локальный `touchedGround` создаёт независимый impact-экземпляр, а рост серверного `groundTouchSeq` воспроизводит все новые контакты из разницы sequence. Поэтому повторные отскоки озвучиваются без нового захвата рукой, покой не создаёт дубликатов, а пропущенный промежуточный snapshot не теряет события.
...
- Shared room settings schema — `32`, settings schema — `34`, localStorage — `sisyphus-czar-settings-v34`. `preclickParallaxActivationRadiusVw` и `preclickHopMaxDistanceVw` имеют диапазон `0–200`; defaults `50/62.5`, у длины шаг UI `0.1`. Начальный `preclickParallaxMaxOffsetVw` — `0–150`, default `0.6`; конечный максимум — `0–50`, default `0`; начальная/конечная задержки — `0–1000 ms`, defaults `0/1000`; transition duration — `1–30 s`, default `30`. `rockGrabRadiusVh` — `0–10`, default `0`. Room payload `<32` и settings payload `<34` без нового ключа получают `clamp(1.25 × preclickParallaxActivationRadiusVw, 0, 200)`; явно заданный новый ключ имеет приоритет. Старые px-поля и `draftFold*` продолжают мигрировать, а `v33` читается как legacy localStorage.
...
- В группе «Камень» находятся два checkbox, три range-контрола автоматического прыжка, одиннадцать baseline-parallax контролов и отдельная «Максимальная длина отскока, vw» для экспериментального hop.
...
- `glowOptimizationMode`, `glowTargetFps`, `glowBufferScalePercent`, `glowUpdateFps`, `glowMaxPoints` и `glowDecimation` имеют `scope: "local"`: сохраняются в v34, но фильтруются из version snapshots, server templates и broadcast.
...
- `settings.update` использует schema `34` и optimistic `settingsRevision`.
...
- UI/Fold smoke проверяет миграции `v32 draftFold* → v34 fold*` и `v33 → v34` для длины отскока, а также основной Fold/UI контракт. Известный старый settings-page round-trip остаётся отдельной нестабильностью полного набора и не используется как доказательство hop-механики.
- Dev smoke проверяет объединённое сохранение именованной версии и комнаты, отсутствие дубликатов в общем каталоге и миграцию legacy-настроек в v34, включая px→vw, Fold, SVG-курсор и `preclickHopMaxDistanceVw`.
