...
- `glowOptimizationMode`, `glowTargetFps`, `glowBufferScalePercent`, `glowUpdateFps`, `glowMaxPoints` и `glowDecimation` имеют `scope: "local"`: сохраняются в v33, но фильтруются из version snapshots, server templates и broadcast.
...
- По умолчанию `EXPERIMENT_PRECLICK_ROCK_HOP` включён: каждый новый вход мыши в радиус выполняет один накопительный hop от текущего визуального центра. Скорость входа `0–2000 px/s` линейно отображается в `0.35–1.25` радиуса; landing ограничивается viewport, новая позиция становится базой следующего входа, а выход не возвращает камень. CSS использует `400 ms cubic-bezier(0.22, 1, 0.36, 1)`, reduced motion — мгновенную посадку. Resize выполняет restart/clamp без дополнительного смеха; первый захват сбрасывает offset и завершает guidance. Каждый hop создаёт независимый `Audio` из `assets/audio/Смех.mp3`, который очищается по `ended`/ошибке; dispose/restart очищают оставшиеся элементы. Явное build-time значение `false` включает baseline-ветку прежнего parallax.
...
- `settings.update` использует schema `33` и optimistic `settingsRevision`.
...
- UI/Fold smoke проверяет единственную compact-кнопку сохранения, применение room settings и отсутствие отдельного `.settings-room-save`, SVG `handopen/handgrabbing` и размер для локальной/remote-руки, основную Fold-сцену на `/`, dev-404 legacy адресов, миграцию `v32 draftFold* → v33 fold*`, диапазон максимума parallax в `vw`, режим постоянной/hover-руки, сохранение preclick и настроек при reload до первого захвата, пользовательский restart, camera lerp, obstacle defaults, popup-blocked UI, glow-профиль, select-зависимости и безопасную структуру зеркала.
- Dev smoke проверяет объединённое сохранение именованной версии и комнаты, отсутствие дубликатов в общем каталоге, миграцию legacy-настроек в v33, включая перевод parallax-радиуса и максимума из px в vw, defaults SVG-курсора `off / 32 px`, defaults динамического перехода, default-off для инверсии, default-on для постоянной руки, лимит glow-точек, отсутствие проходов при `glow=0` и копирование Fold только при новой canvas revision.
...
- Отдельный preclick-hop smoke запускает dev с включённым build-флагом и проверяет один hop на вход, отсутствие повторов внутри радиуса, направление, медленную/быструю дальность, накопление, отсутствие возврата, resize clamp, reduced motion, один смех на hop и завершение первым захватом.
- Collaboration smoke разделяет «Капель» и аудио камня: отдельный сценарий проверяет primary/right/middle, наложение gachi, остановку всего набора именно при `FALLING`, отсутствие impact-аудио в начале падения, один `СимуляцияОргазма.mov` на каждый новый локальный контакт, покой без повтора и дедупликацию shared `groundTouchSeq`. Также он проверяет доступную с клавиатуры кнопку «Начать сначала», нижний viewport suspended-сессии и восстановление активной сессии с видимым камнем после reload.
