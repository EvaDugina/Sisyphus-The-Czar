...
- **Уточнение по обратной связи:** perspective и вращения удалены — parallax меняет только позицию. Максимальное смещение (`0–100 px`, default `12`), круговой радиус от визуального центра камня (`0–2000 px`, default `1000`), duration возврата (`0–2000 мс`, default `400`) и его cubic-bezier-график вынесены в versioned UI-настройки. После выхода указателя за радиус камень плавно возвращается к исходной позиции, а рука остаётся под указателем. Над settings-toggle и открытой settings-panel фото-рука скрывается и отображается нативный курсор. Любой primary `pointerdown` на fine pointer показывает захвативший спрайт руки до `pointerup`/`pointercancel`, включая клики вне камня.
...
| Настраиваемые radius/max/return | Unit/integration + Playwright draft и experiment | PASS | `178/178`; targeted draft `2/2`, включая миграцию старого default `480→1000 px`, UI `12→36 px`, `1000→1400 px`, duration `400→650 мс` и cubic-bezier-график; experiment `1/1` проверяет half-radius, плавный выход за радиус, руку под указателем, неизменность размеров и первый click. |
...
| Default-off regression после настройки | Production smoke | PASS | `2/2`; baseline без экспериментального flag сохранён после settings schema `26` / room schema `22`. |
