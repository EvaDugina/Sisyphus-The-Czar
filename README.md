...
Один участник держит камень, остальные наблюдают и могут взять его после отпускания. Физика общая; production-клиент и production-сервер используют единый preset `prod` из репозитория, а UI параметров остаётся только в dev-сборке. При старте dev UI загружает наиболее поздний сохранённый шаблон по `updatedAt`. Reload/reconnect и пересоздание контейнера сохраняют серверное состояние общей комнаты в Docker volume. Production-старт синхронизирует только настройки root-комнаты с preset, не сбрасывая состояние или общее время. После выхода последнего участника root-комната остаётся жить, поэтому общее время и состояние не начинаются заново при следующем открытии `/`.
