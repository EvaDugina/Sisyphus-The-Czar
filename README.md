...
До первого захвата работает сцена «кошки-мышки»: первые два входа мыши в радиус гарантированно заставляют камень ускакивать от курсора, третий пропускается, а последующие входы используют настраиваемую вероятность несрабатывания `0–100%`. Дистанция зависит от скорости указателя и максимума `0–150%`, а длительность — от заданной скорости камня `100–5000 px/s` и cubic-bezier-кривой. Каждый реально выполненный автоматический или вызванный фейковым кликом отскок запускает независимый экземпляр `Смех.mp3`; пропущенное срабатывание остаётся без звука. Первые N фейковых кликов отталкивают камень без записи trail и не запускают физику; отдельный gachi-звук для них не воспроизводится. Через настраиваемые `0–1000 ms` (default `200 ms`) каждый такой клик открывает popup с очередной картиной `assets/gogh/01.png`–`03.png`: базовая ширина берётся из параметра «Размер после запуска физики» и умножается на «Размер окон с картинами» (`1–4`, default `2×`), после чего окно пропорционально ограничивается рабочей областью экрана. Клиентская область точно повторяет пропорции картины без пустых полей, а окно остаётся открытым до ручного закрытия пользователем. Следующий клик активирует сцену 2, где trail работает согласно настройкам. После активации повторный захват снизу возвращает камень к начальному размеру; lerp-слежение камеры вверх и вниз включено по умолчанию и независимо управляется двумя UI-переключателями, а вертикальный overflow по умолчанию скрыт. В сцене 2 каждый клик по камню запускает независимый экземпляр выбранного gachi-звука с default `Camen.mp3`; активные экземпляры накладываются и доигрывают естественно. На вершине отображаются `The Path of Tzarey` и `miniature` локальным шрифтом Comico Regular из `assets/fonts`.
