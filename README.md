...
На `/scene-1` работает сцена «кошки-мышки»: первые два входа мыши в радиус гарантированно заставляют камень ускакивать от курсора, третий пропускается, а последующие входы используют настраиваемую вероятность несрабатывания `0–100%`. Ровно два фейковых клика отталкивают камень с выбранным в UI звуком (`Без звука`, `Смех.mp3` или один из gachi-файлов), не записывают обычную траекторию и не запускают физику; их количество зафиксировано и не настраивается. После настраиваемой задержки они открывают popup с картинами из `assets/gogh` в одном из трёх режимов: независимый случайный выбор, строгий цикл `01 → 02 → 03 → 01` или одна выбранная картина. Ширина popup задаётся долей viewport `0.01–1` с шагом `0.01`, высота сохраняет natural aspect ratio картины. Включённый пульс камня работает уже в preclick-состоянии сцены 1. Следующий, настоящий pointerdown создаёт два новых окна — одно в точке клика и второе в случайной позиции, показывает их вместе со всеми живыми окнами картин, оставляет фото-руку сжимающей камень и фиксирует финал сцены 1. Через сохраняемую задержку `0–5 с` с шагом `0.1` единственный таймер убирает все живые popup с переднего плана, не закрывая их, и возвращает фокус главному окну; дополнительные настоящие клики таймер не меняют. Движение камня оставляет отдельный настраиваемый эхо-след из перекрывающихся затухающих копий. `/scene-2` независимо стартует с подвешенным камнем в центре, сохраняет захват при первом клике, а при контакте с отпечатком на вершине магнитно центрирует камень и доскролливает камеру наверх. `/scene-3` сразу показывает вершину и WebGL-линзу на `rock-03`: доступны Brandon Mercer Flowmap, Liquid Bulge, Vortex Lens, Pinch Tunnel и Ripple Glass с контролами радиуса и восстановления. При первом попадании в отпечаток камень магнитно центрируется и остаётся серверно зафиксированным до настоящего scroll вниз; затем выполняются падение, спуск камеры и дождь. Параметры каждой страницы находятся рядом со сценой: уникальные значения сохраняются отдельно, а 20 общих контролов сцен 1–3 и ещё 64 общих контрола сцен 2–3 помечены badge «Общий» и используют последнее сохранённое значение на всех применимых страницах после reload.

В сцене 2 включённый пульс работает до первого захвата, после выпадения и после приземления, но останавливается на время сжатия камня рукой. «Звук клика по камню» и «Звук касания боковой стены» выбираются независимо и оба поддерживают `Без звука`. Обычные `pointerup`/`pointercancel` не размыкают текущий захват: камень освобождается только включённым «Случайным выпадением» или «Выпрыгиванием вверх». После фактического выпадения runtime снимает sticky-защёлку, поэтому камень можно снова захватить.
...
Если `Fhand < m · g`, рука не отпускает камень: он плавно отстаёт со скоростью, зависящей от `Fhand / Fg`. В группе «Камень» независимо включаются случайное выпадение через `0.5–2 s` и выпрыгивание вверх. Для выпрыгивания задаются интервал `1–10 s` (default `5`), полная ширина симметричного углового сектора `0–180°` (default `90°`) и разброс силы `0–1%` с шагом `0.01` (default `0.25%`). Параметр входа в нижнюю и боковые стены имеет диапазон `0–50%` и default `20%`; верхняя граница не изменяется.

Свечение траектории вынесено из базовой линии в отдельный canvas и рисуется одним ограниченным blur-проходом. В UI доступны локальные профили `Авто`, `Производительность`, `Баланс`, `Качество` и `Ручной`; ручной режим задаёт масштаб glow-буфера, частоту обновления, лимит точек и прореживание. Эти параметры сохраняются в общем browser snapshot всех UI-настроек, а после нажатия кнопки сохранения входят в серверный шаблон и production preset. Неактуальные поля автоматически получают `disabled` без сброса сохранённых значений.

Production frontend использует `rock-03.png`, PNG-курсоры `cursor-grab-02.png`/`cursor-grabbing-02.png` единственной руки и lazy-загрузку MP3. Рука имеет режимы `always/hover/hidden`, а смена grab-спрайта может быть отложена на `0–1000 ms` без задержки физики. В сцене 3 фото-рука заменена компактными `assets/cursor/handopen.svg` / `handgrabbing.svg` размером `32 px`. Регулярные snapshots отправляются без неизменных `physics`, `roomSettings`, `imprint` и `expiresAt`; для сцены 3 дополнительно синхронизируется `sceneThreeLocked`. Частоты: до 30 Hz input и до 20 Hz snapshots.
