...
До первого захвата работает сцена «кошки-мышки»: первые два входа мыши в радиус гарантированно заставляют камень ускакивать от курсора, третий пропускается, а последующие входы используют настраиваемую вероятность несрабатывания `0–100%`. Дистанция зависит от скорости указателя и максимума `0–150%`, а длительность — от заданной скорости камня `100–5000 px/s` и cubic-bezier-кривой. Автоматические отскоки проходят без смеха; первые N фейковых кликов открывают центрированное пустое окно в текущих пропорциях камня, включают `Смех.mp3`, отталкивают камень без записи trail и не запускают физику. Следующий клик без gachi активирует сцену 2, где trail работает согласно настройкам. После активации повторный захват снизу возвращает камень к начальному размеру; lerp-слежение камеры вверх и вниз включено по умолчанию и независимо управляется двумя UI-переключателями, а вертикальный overflow по умолчанию скрыт. Каждый клик по камню запускает независимый экземпляр выбранного gachi-звука с default `Camen.mp3`; активные экземпляры накладываются и доигрывают естественно. На вершине отображаются `The Path of Tzarey` и `miniature` локальным шрифтом Comico Regular из `assets/fonts`.
