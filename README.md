...
До первого захвата работает основная сцена «кошки-мышки»: каждый новый вход мыши в радиус заставляет камень ускакивать от курсора без возврата в центр и запускает независимый `Смех.mp3`. В общей группе «Камень» поля «Радиус срабатывания, vw» и «Максимальная длина отскока, vw» независимо задают вход и дальность; скорость `0–2000 px/s` даёт `28–100%` выбранного максимума. Анимация длится `400 ms`, а центр камня бесшовно переносится между левым/правым и верхним/нижним краями с сохранением остатка пути. Первый захват материализует видимую позицию, завершает сцену и включает физику с camera-follow.
