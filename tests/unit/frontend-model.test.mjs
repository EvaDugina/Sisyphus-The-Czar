...
test("параметры формул подъёма и падения начинаются с 0.1 в UI", () => {
  ...
  assert.deepEqual(
    ...
    { min: 0.1, max: 100, step: 0.1, defaultValue: 4 }
  );
  assert.deepEqual(
    {
      min: gravity.min,
    ...
    { min: 0.1, max: 10, step: 0.05, defaultValue: 0.45 }
  );
  assert.deepEqual(
    {
      min: handForce.min,
    ...
    { min: 0.1, max: 10, step: 0.1, defaultValue: 5 }
  );
});
