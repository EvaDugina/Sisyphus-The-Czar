...
test("Git production preset содержит полный canonical snapshot", () => {
  ...
  assert.equal(normalized.source.settingsSchemaVersion, 48);
  ...
  assert.equal(normalized.settings.upperZoneAutoScrollEnabled, false);
...
test("production preset мигрирует legacy Fold-ключи и длину отскока в schema 48", () => {
