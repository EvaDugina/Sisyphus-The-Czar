...
test("Git production preset содержит полный canonical snapshot", () => {
  ...
  assert.equal(normalized.source.settingsSchemaVersion, 45);
  ...
  assert.equal(normalized.settings.upperZoneAutoScrollEnabled, true);
...
test("production preset мигрирует legacy Fold-ключи и длину отскока в schema 45", () => {
