...
test("отпечаток останавливает камень только после первого падения", () => {
  const imprint = {
    x: 500,
    y: 700,
    toleranceX: 30,
  ...
  assert.equal(Physics.finishAtImprint(falling, imprint), false);
  ...
  assert.equal(Physics.finishAtImprint(playing, imprint), false);

  playing.x = 529;
  assert.equal(Physics.finishAtImprint(playing, imprint), true);
  assert.equal(playing.phase, Physics.PHASES.WON);
  assert.equal(playing.x, imprint.x);
  assert.equal(playing.y, imprint.y);
  assert.equal(playing.vx, 0);
  assert.equal(playing.vy, 0);
});
