...
test("верхняя зона останавливает камень только после первого падения", () => {
  ...
  assert.equal(Physics.finishAtStopZone(falling, 800), false);
  ...
  assert.equal(Physics.finishAtStopZone(playing, 800), true);
  assert.equal(playing.phase, Physics.PHASES.WON);
  assert.equal(playing.y, 800);
