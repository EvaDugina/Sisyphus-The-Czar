...
test("инерция использует шкалу от 0 до 10", () => {
  assert.equal(Physics.sanitizePhysics({ inertia: -1 }).inertia, 0);
  assert.equal(Physics.sanitizePhysics({ inertia: 7.5 }).inertia, 7.5);
  assert.equal(Physics.sanitizePhysics({ inertia: 11 }).inertia, 10);
});
...
test("первое падение начинается вниз без импульса руки", () => {
  ...
  const physics = Physics.sanitizePhysics({ inertia: 10, turbulence: 0 });
...
test("инерция масштабирует импульс и сохраняет направление движения руки", () => {
  ...
  Physics.applyReleaseImpulse(
    half,
    Physics.sanitizePhysics({ ...base, inertia: 5 }),
    300,
    -400
  );
  Physics.applyReleaseImpulse(
    full,
    Physics.sanitizePhysics({ ...base, inertia: 10 }),
    300,
    -400
  );
  Physics.applyReleaseImpulse(
