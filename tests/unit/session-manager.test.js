...
test("первое касание сохраняет отпечаток и возвращение останавливает камень", () => {
  const { manager } = setup();
  const session = manager.createSession({
  ...
  assert.equal(session.state.x, session.imprint.x);
  assert.equal(session.state.y, session.imprint.y);
  assert.equal(session.state.dragging, false);
  assert.equal(session.state.controllerId, null);
});

test("брошенный камень останавливается при попадании в отпечаток", () => {
  const { clock, manager } = setup();
  const session = manager.createSession({
